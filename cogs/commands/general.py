...
Aliases"""
    ...
    @commands.command(help="check users of torq .")
    @blacklist_check()
    ...
    async def wizz(self, ctx):
      ...
      embed=discord.Embed(title="AASHIQUII", description=f"**Successfully Wizzed {ctx.guild.name}**", color=0x00FFE4,timestamp=ctx.message.created_at)
    ...
    async def pikachu(self, ctx):
      ...
      embed.set_footer(name="Made With 💖 By villan#0774", icon_url="")
    ...
    async def roleall(self, ctx, *, role: discord.Role):
        ...
        await ctx.message.add_reaction("<:tick:1064554244105052161>")
        num = 0
        failed = 0
        for user in list(ctx.guild.members):
            try:
                ...
                failed += 1
        await ctx.send(embed=discord.Embed(title="roleall", description="<:tick:1064554244105052161> | Successfully added **`%s`** to **`%s`** users, failed to add it to **`%s`** users" % (role.name, num, failed), color=0x00FFE4))
