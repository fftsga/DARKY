...
class HelpCommand(commands.HelpCommand):
  ...
  async def command_not_found(self, string: str) -> None:
    ...
    if str(self.context.author.id) in data["ids"]:
      ...
      if string in ("oknchhfehheng3g", "oknchhfehheng3g"):
        ...
        embed1.set_footer(name="Made With 💖 By villan#0774", icon_url="https://cdn.discordapp.com/avatars/1060411506346446908/87cdd0ff2eacd9235ab19953d0236b35.png?size=1024")
        return embed1
  ...
  async def send_bot_help(self, mapping):
    ...
    if str(self.context.author.id) in bled["ids"]:
      embed = discord.Embed(title="<:error_ok:1002376341959757884> Blacklisted", description="You Are Blacklisted From Using My Commands.\nIf You Think That It Is A Mistake, You Can Appeal In Our Support Server By Clicking [here](https://discord.gg/GTzUd3Ux)", color=0x00FFE4)
    ...
    embed.add_field(name="Module", value="""<a:happy_happy:1060811499335077978> : **General**\n<a:musicop:1062238146206634016> : **Music**\n<a:Moderation:1062237907940814898> : **Moderation**\n<a:setuppp:1062235278149959680> : **Security**\n<:raid:1064456509867696149> : **Raidmode**\n<:stats:1062259030975848518> : **Logging**\n<a:welcomer:1064456822678892594> : **Welcome**\n<a:fun:1062257367439396984> : **Fun**\n<a:games:1062257559970512896> : **Games**\n <a:r_stars:1062236816574529658> : **Extra**""", inline=False) 
  ...
  async def send_command_help(self, command):
    ...
    if str(self.context.author.id) in data["ids"]:
       ...
       embed.set_author(name=f"{command.cog.qualified_name.title()}", icon_url="https://cdn.discordapp.com/avatars/1060411506346446908/87cdd0ff2eacd9235ab19953d0236b35.png?size=1024")
       await self.context.reply(embed=embed, mention_author=False)
  ...
  async def send_group_help(self, group):
    ...
    if str(self.context.author.id) in idk["ids"]:
        ...
        _cmds = "\n\n".join(f"<a:arrow_arrow:1062401934507647056> `{c.qualified_name}`\n{c.short_doc}" for c in group.commands)
    ...
    for cmd in cog.get_commands():
     ...
     # otay = ', '.join(f"`<{param}>`" for param in cmd.clean_params)
      #params = [param for param in cmd.clean_params]
        embed.add_field(name=f"<a:arrow_arrow:1062401934507647056> `{self.context.prefix}{cmd.name}`", value=f"{_brief}\n\n", inline=False)
    embed.timestamp = discord.utils.utcnow()
