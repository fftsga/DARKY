...
class Raidmode(Cog):
  ...
  async def _antiraid(self, ctx):
    ...
    embed = discord.Embed(title="AASHIQUII | Antiraid",
        ...
        if ctx.author == owner:
            if onOroff == "on":
                if data["antiSpam"] is True:
                    hacker = discord.Embed(title="AASHIQUII | Antiraid",
                          description=f"<a:wronggg:1064554853432557591> | Anti-Spam is already enabled for **`{ctx.guild.name}`**",color=0x00FFE4)  
                    await ctx.reply(embed=hacker, mention_author=False)
                else:
                    data["antiSpam"] = True
                    updateConfig(ctx.guild.id, data)
                    hacker1 = discord.Embed(title="AASHIQUII | Antiraid",
                          description=f"<:tick:1064554244105052161> | Successfully enabled anti-spam for **`{ctx.guild.name}`**",color=0x00FFE4)  
                ...
                data["antiSpam"] = False
                updateConfig(ctx.guild.id, data)
                hacker2 = discord.Embed(title="AASHIQUII | Antiraid",
                          description=f"<:tick:1064554244105052161> | Successfully disabled anti-spam for **`{ctx.guild.name}`**",color=0x00FFE4)  
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(title="AASHIQUII | Automoderation",
                          description=f"<a:wronggg:1064554853432557591> | Invalid Type.\nIt Should Be On/Off",color=0x00FFE4)  
        ...
        if ctx.author == owner:
            if onOroff == "on":
                if data["antiLink"] is True:
                    hacker = discord.Embed(title="AASHIQUII | Antiraid",
                          description=f"<a:wronggg:1064554853432557591> | Anti-link is already enabled for **`{ctx.guild.name}`**",color=0x00FFE4)  
                    await ctx.reply(embed=hacker, mention_author=False)
                else:
                    data["antiLink"] = True
                    updateConfig(ctx.guild.id, data)
                    hacker1 = discord.Embed(title="AASHIQUII | Antiraid",
                          description=f"<:tick:1064554244105052161> | Successfully enabled anti-link for **`{ctx.guild.name}`**",color=0x00FFE4)  
                    await ctx.reply(embed=hacker1, mention_author=False)
                ...
                updateConfig(ctx.guild.id, data)
                hacker2 = discord.Embed(title="AASHIQUII | Antiraid",
                          description=f"<:tick:1064554244105052161> | Successfully disabled anti-link for **`{ctx.guild.name}`**",color=0x00FFE4)  
                await ctx.reply(embed=hacker2, mention_author=False)
            else:
                hacker3 = discord.Embed(title="AASHIQUII | Antiraid",
                          description=f"<a:wronggg:1064554853432557591> | Invalid Type.\nIt Should Be On/Off",color=0x00FFE4)  
                await ctx.reply(embed=hacker3, mention_author=False)
