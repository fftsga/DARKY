...
async def on_member_join(member):
    embed = discord.Embed(title="Do you own a server?", description=f"AASHIQI offers  the fastest server protection available, with a powerful **anti-nuke**, **anti-spam**, **anti-link**, **Moderation**, **Logging** and time passing thing **Games**, i can protect your server in multiple ways today.\n\nThat's why **{member.guild.name}** and **{len(client.guilds)}** other servers use me to protect themselves!")
    embed.set_thumbnail(url=member.avatar)
    embed.set_footer(text="Made With 💖 By villan#0774", icon_url=client.user.avatar.url)
    embed.set_author(name="AASHIQUII", icon_url=client.user.avatar.url)
    await member.send(f"- Sent From {member.guild.name}\https://discord.gg/aashiquii",embed = embed , mention_author = True)
